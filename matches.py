...
df = pandas.read_csv('data/atp_matches_2018.csv')
...
df = df.rename(
    columns={'tourney_date': 'date', 'winner_ioc': 'winner_state', 'loser_ioc': 'loser_state'})

df['date'] = df['date'].apply(
    lambda x: pandas.to_datetime(str(x), format='%Y%m%d'))

df.to_csv('data/2018.csv', index=False)
