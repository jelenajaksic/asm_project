...
rank2020['date'] = rank2020['date'].apply(
...
rank2019 = rank2019.merge(players, how='inner',
                          left_on='player', right_on='id')
print(rank2019.columns)
rank2019['name'] = rank2019['name'] + ' ' + rank2019['surname']
rank2019 = rank2019.drop(columns=['hand', 'birth', 'surname'])

rank2019 = rank2019.rename(
    columns={'ranking_date': 'date', 'player': 'id', 'id': 'b'})
rank2019 = rank2019.drop(columns=['b'])
rank2019['date'] = rank2019['date'].apply(
    lambda x: pandas.to_datetime(str(x), format='%Y%m%d'))
